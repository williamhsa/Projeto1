...
public class Server {
	...
	public void start() {
		...
		try 
		{
			...
			while(keepGoing) 
			{
				display("Servidor esperando por clientes na porta " + port + ".");
			...
			for(int y=al.size(); --y>=0;)
			{
				...
				if(check.equals(tocheck))
				{
					...
					if(!ct1.writeMsg(messageLf)) {
						al.remove(y);
						display("Cliente desconectado " + ct1.username + " removido da lista.");
					}
		...
		else
		{
			...
			for(int i = al.size(); --i >= 0;) {
				...
				if(!ct.writeMsg(messageLf)) {
					al.remove(i);
					display("Cliente desconectado " + ct.username + " removido da lista.");
				}
			}
		}
		return true;
	...
	synchronized void remove(int id) {
		...
		broadcast(notif + disconnectedClient + " se desconectou." + notif);
	}
	...
	public static void main(String[] args) throws IOException {
		...
		Server server = new Server(conexao.getValue());
		server.start();
	}
	...
	class ClientThread extends Thread {
		...
		ClientThread(Socket socket) {
			...
			try
			{
				...
				broadcast(notif + username + " se juntou a conversa." + notif);
			}
			catch (IOException e) {
